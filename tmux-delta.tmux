...
[[ -z "$_delta_existing" ]] && tmux set-option -g @tmux_delta_apex_icon '󰚩'
...
# ─── Catppuccin PR author module ─────────────────────────────────────────────
# Shown only when the current branch has an open PR. Populated by
# tmux-git-status-refresh.sh into @git_pr_author_cache.
tmux set-option -gF  @catppuccin_status_pr_author "#[fg=${C_GREEN}]#[bg=default]${SEP_L}#[fg=${C_CRUST},bg=${C_GREEN}]"
tmux set-option -ag  @catppuccin_status_pr_author '󰀄 '
tmux set-option -agF @catppuccin_status_pr_author "#[fg=${C_FG},bg=${C_SURFACE0}] "
tmux set-option -ag  @catppuccin_status_pr_author '#{@git_pr_author_cache}'
tmux set-option -agF @catppuccin_status_pr_author "#[fg=${C_SURFACE0}]#[bg=default]${SEP_R}"
...
status_fmt0="#{S/n:#[range=session|#{session_id}]#[fg=${PILL_BG_INACTIVE}]#[bg=default]${SEP_L}#[fg=${PILL_FG_INACTIVE} bg=${PILL_BG_INACTIVE}]#{?#{==:#{@session_type},folder},󰉋,#{?#{==:#{@session_type},worktree},󰘬,󰊢}} #{?#{@session_label},#{@session_label},#{session_name}}#{?#{==:#{@apex_role},manager}, #[fg=${C_PINK}]${APEX_ICON}#[fg=${PILL_FG_INACTIVE}],}#{?#{@pr_icons}, #{@pr_icons},}#{?#{@agent_working}, #[fg=${C_PEACH}]󰚩#[fg=${PILL_FG_INACTIVE}],} #[fg=${PILL_BG_INACTIVE}]#[bg=default]${SEP_R}#[fg=default bg=default]#[norange] ,#[range=session|#{session_id}]#[fg=${PILL_BG_ACTIVE}]#[bg=default]${SEP_L}#[fg=${C_CRUST} bg=${PILL_BG_ACTIVE}]#{?#{==:#{@session_type},folder},󰉋,#{?#{==:#{@session_type},worktree},󰘬,󰊢}} #{?#{@session_label},#{@session_label},#{session_name}}#{?#{==:#{@apex_role},manager}, #[fg=${C_CRUST}]${APEX_ICON}#[fg=${C_CRUST}],}#{?#{@pr_icons}, #{@pr_icons},}#{?#{@agent_working}, #[fg=${C_PEACH}]󰚩#[fg=${C_CRUST}],} #[fg=${PILL_BG_ACTIVE}]#[bg=default]${SEP_R}#[fg=default bg=default]#[norange] }#[align=right]#{E:status-right}"
